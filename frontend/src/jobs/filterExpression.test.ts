...
describe("expressionToRules", () => {
  ...
  it("marks empty AND as non-renderable", () => {
    const expr = { op: "and" as const, children: [] };
    const result = expressionToRules(expr);
    expect(result.renderable).toBe(false);
  });

  it("marks AND with missing children as non-renderable", () => {
    const expr = { op: "and" as const };
    const result = expressionToRules(expr);
    expect(result.renderable).toBe(false);
  });

  it("treats null children as non-predicate and falls through to non-renderable", () => {
    const expr = {
      op: "and" as const,
      children: [null],
    } as unknown as Parameters<typeof expressionToRules>[0];
    expect(expressionToRules(expr).renderable).toBe(false);
  });

  it("treats primitive children as non-predicate and falls through to non-renderable", () => {
    const expr = {
      op: "and" as const,
      children: [42],
    } as unknown as Parameters<typeof expressionToRules>[0];
...
describe("rulesToExpression", () => {
  ...
  it("handles null value on rule", () => {
    const rules: FilterRule[] = [
      { id: "r1", field: "title", operator: "contains", value: null },
    ];
    expect(rulesToExpression(rules)).toBe(null);
  });
});
...
describe("containsDisjunctionOrNegation", () => {
  ...
  it("returns true for NOT group", () => {
    expect(
      containsDisjunctionOrNegation({
        op: "not",
        child: { field: "a", operator: "eq", value: "x" },
      }),
    ).toBe(true);
  });
